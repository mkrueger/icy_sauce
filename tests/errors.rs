use bstr::BString;
use icy_sauce::{SauceInformation, SauceInformationBuilder};

#[test]
fn test_empty_sauce() {
    // Test reading data with no SAUCE
    let data = b"Just some data without SAUCE";
    let result = SauceInformation::read(data).unwrap();
...
fn test_comments_without_valid_id() {
    ...
    let info = SauceInformationBuilder::default()
        ...
        .unwrap()
        .with_comment(BString::from("Comment 1"))
        .unwrap()
        .build();
...
fn test_maximum_comments() {
    ...
    for i in 0..255 {
        builder = builder
            .with_comment(BString::from(format!("Comment {}", i)))
            .unwrap();
    }

    // Adding 256th comment should fail
    let result = builder.with_comment(BString::from("One too many"));
